# Code taken from https://github.com/cisnlp/MaskLID (Kargaran et al., 2024b)
import fasttext
import numpy as np
import re
import string
...
class MaskLID:
    ...
    def _scores(self, x):
        """Turn raw logits into per-label scores matching the training loss.
        Args:
            x (numpy.ndarray): Raw logits for the selected languages.

        Returns:
            numpy.ndarray: Per-label scores.
        """
    ...
    def predict(self, text, k=1):
        ...
        scores = self._scores(result_vector[self.language_indices])  # !!!!
        top_k_indices = np.argsort(scores)[-k:][::-1]
        top_k_labels = [self.labels[i] for i in top_k_indices]
        top_k_probs = scores[top_k_indices]
